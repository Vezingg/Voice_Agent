...
def resample_mono_pcm16(pcm, src_rate, dst_rate):
    ...
    out_count = max(1, int(round(count * dst_rate / src_rate)))
    out = []
    last = max(out_count - 1, 1)
    ...
    return struct.pack("<" + "h" * out_count, *out)


def mono_to_stereo_pcm16(pcm):
    count = len(pcm) // 2
    samples = struct.unpack("<" + "h" * count, pcm[:count * 2])
    stereo = []
    for sample in samples:
        ...
        stereo.append(sample)
    return struct.pack("<" + "h" * (count * 2), *stereo)


def write_wav_bytes(pcm, sample_rate, channels):
    data_size = len(pcm)
...
def to_browser_wav(data, src_rate=24000):
    """Build a CD-style 44.1 kHz 16-bit stereo WAV Chromium can actually play."""
    pcm = data
    rate = src_rate
    channels = 1
    if data[:4] == b"RIFF":
    ...
    if channels == 2:
        ...
        stereo = struct.unpack("<" + "h" * (frame_count * 2), pcm[:frame_count * 4])
        mono = [
            int((stereo[i] + stereo[i + 1]) / 2)
            for i in range(0, len(stereo), 2)
        ]
        pcm = struct.pack("<" + "h" * len(mono), *mono)
    pcm = resample_mono_pcm16(pcm, rate, 44100)
    pcm = mono_to_stereo_pcm16(pcm)
    return write_wav_bytes(pcm, 44100, 2)


def audio_bytes_duration_seconds(data):
...
def speak():
    ...
    try:
        ...
        audio_bytes = to_browser_wav(concat_wavs(wav_chunks))
        ...
        output_audio_seconds = audio_bytes_duration_seconds(audio_bytes)
